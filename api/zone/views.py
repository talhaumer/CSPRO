...
from api.zone.models import Countries, Zone, ZoneCountries, query_zone_by_args
from api.zone.serializers import (
    CountriesSerializer,
    UserZoneCountrySerializer,
    ZoneCountrySerializer,
    ZoneSerializer,
        ...
        try:
            ...
            if user_zone == True:
                ...
                return self.send_response(
                    ...
                    payload=res,
                    code="200",
                    description="User's zone Countries",
                    log_description="",
                )
    ...
    def put(self, request, pk=None):
        try:
            ...
            serializer = CountriesSerializer(
                instance=saved_countries, data=data, partial=True
            )
            if serializer.is_valid():
                country_saved = serializer.save()
                return self.send_response(
            ...
            return self.send_response(
                code=f"422",
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                description="Country doesn't exists",
            )
        except FieldError:
            return self.send_response(
                code=f"500",
        ...
        try:
            ...
            if search_value:
                zone = zone.filter(
                    ...
                    | Q(zone_zone_countires__countries__name=search_value)
                ).distinct()
            ...
            if pk is not None:
                ...
                if zone.status:
                    return self.send_response(
                        ...
                        status_code=status.HTTP_200_OK,
                        code="",
                        description="Zone is Enabled Successfuly",
                        log_description="",
                    )
            return self.send_response(
                success=True,
                status_code=status.HTTP_200_OK,
                code="",
                description="Zone is Disabled Successfuly",
                log_description="",
            )
        except Zone.DoesNotExist:
            return self.send_response(
