    ...
    def create(self, validated_data):
        try:
            with transaction.atomic():
                ...
                if training_type:
                    preceptorship.training_type = ConstantData.objects.get(
                        code=training_type
                    )

                if types_of_first_training:
                    preceptorship.types_of_first_training = ConstantData.objects.get(
                        code=types_of_first_training
                    )

                if type_advance_training:
                    preceptorship.type_advance_training = ConstantData.objects.get(
                        code=type_advance_training
                    )

                if specific_training:
                    preceptorship.specific_training = ConstantData.objects.get(
                        code=specific_training
                    )

                if not_implant_regularly:
                    preceptorship.not_implant_regularly = ConstantData.objects.get(
                ...
                status_data = {
                    ...
                    "preceptorship_activity": preceptorship,
...
class PreceptorshipProposalViewSerializer(serializers.ModelSerializer):
    ...
    def get_proctor(self, obj):
        try:
            return PreceptorshipProctorsSerializer(
                obj.preceptorship_porposal.all(), many=True
            ).data
        except:
            return None
            ...
            with transaction.atomic():
                ...
                if len(alternatives_data) != 0:
                    ...
                    if "proctor_user_id" in alternatives_data.keys():
                        qs = PreceptorshipProctors.objects.filter(
                            preceptorship_proposal__status__preceptorship_activity__id=perceptorship_activity_id,
                            status=True,
                        )
                        if qs:
...
class PreceptorshipStatusViewSerializer(serializers.ModelSerializer):
    ...
    def get_alternatives(self, obj):
        try:
            return PreceptorshipProposalViewSerializer(
                obj.alter_preceptorship_porposal.get()
            ).data
        except:
            return None
...
class PreceptorshipViewSerializer(serializers.ModelSerializer):
    ...
    def get_status(self, obj):
        try:
            return PreceptorshipStatusViewSerializer(
                obj.preceptorshipStatus_status.all(), many=True
            ).data
        except:
            return None
    ...
    def get_rem_seats(self, obj):
        try:
            ...
            traine = TraineePreceptorshipProfile.objects.filter(
                preceptorship__id=obj.id, revoke=False
            )
    ...
    def update(self, instance, validated_data):
        ...
        if "hospital_id" in validated_data.keys():
            validated_data["hospital"] = Hospital.objects.get(
                id=validated_data.pop("hospital_id")
            )
        ...
        if "country_id" in validated_data.keys():
            validated_data["country"] = Countries.objects.get(
                id=validated_data.pop("country_id")
            )
...
class PreceptorshipTraineeUploadsSerializers(serializers.ModelSerializer):
    ...
    percevel_driver = serializers.CharField(
    ...
    class Meta:
        ...
        fields = [
            "id",
            "preceptorship_trainee_id",
            "percevel_driver",
            "is_memo_family",
            "is_perceval",
            "is_solo_smart",
            "rate_of_demostration",
            "rate_experince_in_operating_room",
            "rate_hospital_facility",
            "number_of_patients",
            "number_of_perceval",
            "rate_of_experince_overall",
            "rate_level_of_training",
            "are_intrested_learning_more",
            "rate_corcym_support",
            "proctor_report",
            "corcym_suggestion",
        ]
    ...
    def create(self, validated_data):
        try:
            with transaction.atomic():
                ...
                rate_experince_in_operating_room = validated_data.pop(
                    "rate_experince_in_operating_room"
                )
                rate_hospital_facility = validated_data.pop("rate_hospital_facility")
                rate_of_experince_overall = validated_data.pop(
                    "rate_of_experince_overall"
                )
                ...
                preceptor_ship_feedback = PreceptorshipTraineeUploads.objects.create(
                    **validated_data
                )
                preceptor_ship_feedback.rate_of_demostration = Rating.objects.get(
                    code=rate_of_demostration
                )
                preceptor_ship_feedback.rate_experince_in_operating_room = (
                    Rating.objects.get(code=rate_experince_in_operating_room)
                )
                preceptor_ship_feedback.rate_hospital_facility = Rating.objects.get(
                    code=rate_hospital_facility
                )
                preceptor_ship_feedback.rate_of_experince_overall = Rating.objects.get(
                    code=rate_of_experince_overall
                )
                preceptor_ship_feedback.rate_level_of_training = Rating.objects.get(
                    code=rate_level_of_training
                )
                preceptor_ship_feedback.rate_corcym_support = Rating.objects.get(
                    code=rate_corcym_support
                )
    ...
    def update(self, instance, validated_data):
        try:
            if "rate_of_demostration" in validated_data.keys():
                rate_of_demostration = validated_data.pop("rate_of_demostration")
                validated_data["rate_of_demostration"] = Rating.objects.get(
                    code=rate_of_demostration
                )
                instance.rate_of_demostration = validated_data.get(
                    "rate_of_demostration", instance.rate_of_demostration
                )
            ...
            if "rate_experince_in_operating_room" in validated_data.keys():
                ...
                validated_data["rate_experince_in_operating_room"] = Rating.objects.get(
                    code=rate_experince_in_operating_room
                )
                instance.rate_experince_in_operating_room = validated_data.get(
                    "rate_experince_in_operating_room",
            ...
            if "rate_hospital_facility" in validated_data.keys():
                rate_hospital_facility = validated_data.pop("rate_hospital_facility")
                validated_data["rate_hospital_facility"] = Rating.objects.get(
                    code=rate_hospital_facility
                )
                instance.rate_hospital_facility = validated_data.get(
                    "rate_hospital_facility", instance.rate_hospital_facility
                )
            ...
            if "rate_of_experince_overall" in validated_data.keys():
                ...
                validated_data["rate_of_experince_overall"] = Rating.objects.get(
                    code=rate_of_experince_overall
                )
                instance.rate_of_experince_overall = validated_data.get(
                    "rate_of_experince_overall", instance.rate_of_experince_overall
                )

            if "rate_level_of_training" in validated_data.keys():
                rate_level_of_training = validated_data.pop("rate_level_of_training")
                validated_data["rate_level_of_training"] = Rating.objects.get(
                    code=rate_level_of_training
                )
                instance.rate_level_of_training = validated_data.get(
                    "rate_level_of_training", instance.rate_level_of_training
                )

            if "rate_corcym_support" in validated_data.keys():
                rate_corcym_support = validated_data.pop("rate_corcym_support")
                validated_data["rate_corcym_support"] = Rating.objects.get(
                    code=rate_corcym_support
                )
                instance.rate_corcym_support = validated_data.get(
                    "rate_corcym_support", instance.rate_corcym_support
                )

            instance.number_of_patients = validated_data.get(
                "number_of_patients", instance.number_of_patients
            )
            instance.number_of_perceval = validated_data.get(
                "number_of_perceval", instance.number_of_perceval
            )
            instance.are_intrested_learning_more = validated_data.get(
                "are_intrested_learning_more", instance.are_intrested_learning_more
            )
...
class PreceptorshipViewListingSerializer(serializers.ModelSerializer):
    ...
    def get_status(self, obj):
        try:
            return PreceptorshipStatusViewSerializer(
                obj.preceptorshipStatus_status.latest("timestamp")
            ).data["code"]
        except:
            return None
    ...
    def get_proctor(self, obj):
        try:
            ...
            return PreceptorshipProctorsSerializer(
                obj.preceptorshipStatus_status.filter(
                    alter_preceptorship_porposal__isnull=False
                )
                .latest("created_on")
                ...
                .latest("created_on")
                .preceptorship_porposal.get(status=True)
            ).data["proctors"]
        except:
            return None
...
class AttendancePerceptorShipSerailizers(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        try:
            with transaction.atomic():
                attendace_form = AttendanceFormPerceptorship.objects.create(
                    **validated_data
                )
                attendace_form.save()
