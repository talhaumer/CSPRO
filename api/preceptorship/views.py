...
from api.preceptorship.models import Preceptorship, PreceptorshipStatus, PreceptorshipTraineeUploads, \
	TraineePreceptorshipProfile, PreceptorshipProposal, InvoicePerceptorship, AttendanceFormPerceptorship
from api.preceptorship.serializers import PreceptorshipSerializer, PreceptorshipViewSerializer, \
	PreceptorshipStatusSerializer, PreceptorshipStatusViewSerializer, PreceptorshipTraineeUploadsSerializers, \
	TraineePreceptorshipSerializer, TraineeUpdatePreceptorshipSerializer, PreceptorshipTraineeUpdateUploadsSerializers, \
	PProposalDateUpdateSerializer, PreceptorshipViewListingSerializer, InvoicePerceptorshipSerializer, \
	AttendancePerceptorShipSerailizers, PreceptorshipStatusTestingSerializer, InvoicePerceptorshipUpdateSerializer, \
	AttendanceFormPerceptorshipUpdateSerailizers


# Create your views here.
from cspro.utilities.convert_boolean import boolean
from cspro.utils import check, check_perceptorship, check_masterproctorship, check_event, perceptership
...
class PreceptorshipView(BaseAPIView):
	...
	def get(self, request, pk=None):
		try:
			...
			return self.send_response(success=True, status_code=status.HTTP_200_OK, payload=serializer.data, code='',
									  description='Details of serializer', log_description='', count=len(preceptorship))
		...
		try:
			...
			if serializer.is_valid():
				...
				return self.send_response(success=True, code=f'201', payload={'id': id},
										  status_code=status.HTTP_201_CREATED, description='Preceptorship is created')
	...
	def put(self, request, pk=None):
		try:
			...
			data = request.data
			serializer = PreceptorshipSerializer(instance=saved_preceptorship, data=data, partial=True)
			if serializer.is_valid():
				serializer.save()
...
class PreceptorshipStatusView(BaseAPIView):
	...
	def get(self, request, pk=None):
		try:
			...
			if activity_id:
				activity_status = PreceptorshipStatus.objects.filter(preceptorship_activity__id=activity_id).order_by(
					"timestamp")
				serializer = PreceptorshipStatusViewSerializer(activity_status, many=True)
				return self.send_response(success=True, status_code=status.HTTP_200_OK, payload=serializer.data,
										  code='', description='Details of serializer', log_description='')
			return self.send_response(success=False, status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, payload=[],
									  code='422', description='Enter Valid ProctorshipId')
	...
	def post(self, request):
		try:
			...
			data['user_id'] = request.user.id
			start_date = str(
				PreceptorshipProposal.objects.filter(status__preceptorship_activity__id=data["perceptorship_activity_id"]).latest(
					"created_on").start_date)
			end_date = str(PreceptorshipProposal.objects.filter(
				status__preceptorship_activity__id=data["perceptorship_activity_id"]).latest("created_on").end_date)

			if data["status"] == "confirmed":
				proctors = PreceptorshipStatus.objects.filter(preceptorship_activity__id=data["perceptorship_activity_id"]).filter(
					alter_preceptorship_porposal__isnull=False).latest('created_on').alter_preceptorship_porposal.filter(
					preceptorship_porposal__isnull=False).latest('created_on').preceptorship_porposal.all()
...
class PreceptorshipFeedBackView(BaseAPIView):
	...
	def get(self, request, pk=None):
		try:
			...
			if pk:
				activity_status = PreceptorshipTraineeUploads.objects.filter(preceptorship_trainee__preceptorship__id=pk)
				serializer = PreceptorshipTraineeUploadsSerializers(activity_status, many=True)
				return self.send_response(success=True, status_code=status.HTTP_200_OK, payload=serializer.data,
										  code='', description='Details of serializer', log_description='')

			if activityid:
				activity_status = PreceptorshipTraineeUploads.objects.filter(preceptorship_trainee__preceptorship__id=activityid)
				serializer = PreceptorshipTraineeUploadsSerializers(activity_status, many=True)
				return self.send_response(success=True, status_code=status.HTTP_200_OK, payload=serializer.data,
										  code='', description='Details of serializer', log_description='')
	...
	def post(self, request):
		try:
			...
			if serializer.is_valid():
				saved_status = serializer.save()
				perceptership(Preceptorship.objects.get(trainee_preceptorship__id = data["preceptorship_trainee_id"]))
				# trainee_count = TraineePreceptorshipProfile.objects.filter(
				# 	preceptorship__id=data["preceptorship_id"]).count()
				# if InvoicePerceptorship.objects.filter(preceptorship__id=data["preceptorship_id"]) and  AttendanceFormPerceptorship.objects.filter(preceptorship__id=data["preceptorship_id"]) and trainee_count == PreceptorshipTraineeUploads.objects.filter(
				# 					preceptorship_trainee__preceptorship__id=data["preceptorship_id"]).count():
				# 	PreceptorshipStatus.objects.filter(preceptorship_activity__id=data["preceptorship_id"]).update(
				# 		is_active=False)
				# 	status_data = {'status': StatusConstantData.objects.get(code='closed'),
				# 				   'preceptorship_activity': Preceptorship.objects.get(
				# 					   id=data["preceptorship_id"]),
				# 				   'user': request.user}
				# 	status_data = PreceptorshipStatus.objects.create(**status_data)
				# 	status_data.save()
				# 	return self.send_response(success=True, code=f'201', status_code=status.HTTP_201_CREATED,
				# 									  description='Preceptorship Trainee Uploads is created and Perceptorship is closed')
				return self.send_response(success=True, code=f'201', status_code=status.HTTP_201_CREATED,
	...
	def put(self, request, pk=None):
		try:
			data = request.data
			saved_report = PreceptorshipTraineeUploads.objects.get(preceptorship_trainee__id=pk)
			serializer = PreceptorshipTraineeUpdateUploadsSerializers(instance=saved_report, data=data)
			if serializer.is_valid():
				saved_status = serializer.save()
				return self.send_response(success=True, code=f'201', status_code=status.HTTP_201_CREATED,
...
class TraineePreceptorshipProfileView(BaseAPIView):
	...
	def get(self, request, pk=None):
		try:
			...
			if preceptorship_id:
				trainees = TraineePreceptorshipProfile.objects.filter(preceptorship__id=preceptorship_id)
				serializer = TraineePreceptorshipSerializer(trainees, many=True)
				return self.send_response(success=True, status_code=status.HTTP_200_OK, payload=serializer.data,
										  code='', description='Details of serializer', log_description='',
										  count=len(trainees))
			return self.send_response(success=False, status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, payload=[],
									  code='422', description='Enter valid preceptorship Id', log_description='')
		except ObjectDoesNotExist:
			return self.send_response(code='422', status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
...
class TraineePreceptorshipProfileAddView(BaseAPIView):
	...
	def put(self, request, pk=None):
		try:
			...
			serializer = TraineeUpdatePreceptorshipSerializer(instance=saved_trainee, data=data)
			if serializer.is_valid():
				serializer.save()
				return self.send_response(success=True, code=f'201', status_code=status.HTTP_201_CREATED,
										  description='trainee is created')
...
class PreceptorshipLocalGlobalView(BaseAPIView):
	...
	def get(self, request, pk=None):
		try:
			...
			query_object &= Q(Q(preceptorshipStatus_status__status__code="confirmed") |
			...
			if area_type == 'local':
				preceptorship = Preceptorship.objects.filter(is_global=False, zone__id=zone_id, product__id =product )
				preceptorship = preceptorship.filter(query_object).distinct()
				serializer = PreceptorshipViewSerializer(preceptorship, many=True)
				return self.send_response(success=True, status_code=status.HTTP_200_OK, payload=serializer.data,
										  code='', description='Details of serializer', log_description='', count=preceptorship.count())
			else:
				preceptorship = Preceptorship.objects.filter(is_global=True, product__id =product)
				preceptorship = preceptorship.filter(query_object).distinct()
				serializer = PreceptorshipViewSerializer(preceptorship, many=True)
				return self.send_response(success=True, status_code=status.HTTP_200_OK, payload=serializer.data,
										  code='', description='Details of serializer', log_description='',
...
class PDateUpdateView(BaseAPIView):
	...
	def put(self, request, pk=None):
		try:
			...
			serializer = PProposalDateUpdateSerializer(instance=saved_proposal, data=request.data)
			if serializer.is_valid():
				saved_status = serializer.save()
				return self.send_response(success=True, code=f'201', status_code=status.HTTP_201_CREATED,
...
class PreceptorshipListingApiView(BaseAPIView, CustomPagination):
	...
	def get(self, request, pk=None):
		try:
			...
			if search_value:
				perceptorship = perceptorship.filter(Q(id__icontains=search_value) |
													 ...
													 Q(preceptorshipStatus_status__alter_preceptorship_porposal__preceptorship_porposal__proctors__user__name__icontains = search_value)).distinct()
			...
			if actvity_status:
				query_object &= Q(preceptorshipStatus_status__status__code=actvity_status)

			if type_training:
				query_object &= Q(training_type__code=type_training)

			perceptorship = perceptorship.filter(query_object).order_by(self.sorting_data(column_name,order)).distinct()
			# perceptorship =
			# if rev == 'true':
			# 	perceptorship =  perceptorship.reverse()

			# results = self.paginate_queryset(perceptorship, request, view=self)
			if is_dashboard:
				perceptorship = perceptorship.filter(is_global=True)
				perceptorship = perceptorship.exclude(preceptorshipStatus_status__status__code="closed")

			serializer = PreceptorshipViewListingSerializer(perceptorship[offset: offset + limit], many=True)
			return self.send_response(success=True, status_code=status.HTTP_200_OK, payload=serializer.data, code='200',
									  description='Perceptorship Details', log_description='', count=perceptorship.count())
		except Preceptorship.DoesNotExist:
			return self.send_response(code=f'422', status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
...
class InvoicePerceptorshipView(BaseAPIView):
	...
	def get(self, request, pk=None):
		try:
			activityid = request.query_params.get("activity_id", "")
			if pk is not None:
				proctorship_invoice = InvoicePerceptorship.objects.filter(preceptorship__id=pk)
				serializer = InvoicePerceptorshipSerializer(proctorship_invoice, many=True)
			...
			if activityid:
				proctorship_invoice = InvoicePerceptorship.objects.filter(preceptorship__id=activityid)
				serializer = InvoicePerceptorshipSerializer(proctorship_invoice, many=True)
				return self.send_response(success=True,status_code=status.HTTP_200_OK, payload=serializer.data,code= '',description='Details of serializer',log_description='')
			...
			return self.send_response(success=True,status_code=status.HTTP_200_OK, payload=serializer.data,code= '',description='Details of serializer',log_description='', count = proctorship_invoices.count())
		except ObjectDoesNotExist:
			return self.send_response(code='422', status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, description="No ProctorShip Invoice matches the given query.")
		except InvoicePerceptorship.DoesNotExist:
			return self.send_response(code='', status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, description="ProctorShip doesn't exists")
		except FieldError:
			return self.send_response(code=f'500', description="Cannot resolve keyword given in 'order_by' into field")
		except Exception as e:
			return self.send_response( code=f'500',description=e)
	...
	def post(self, request):
		try:
			...
			if serializer.is_valid():
				...
				# count = TraineePreceptorshipProfile.objects.filter(preceptorship__id=data["preceptorship_id"]).count()
				# if AttendanceFormPerceptorship.objects.filter(preceptorship__id=data["preceptorship_id"]) and  PreceptorshipTraineeUploads.objects.filter(
				# 			preceptorship_trainee__preceptorship__id=data["preceptorship_id"]).count() == count:
				# 	PreceptorshipStatus.objects.filter(preceptorship_activity__id=data["preceptorship_id"]).update(
				# 		is_active=False)
				# 	status_data = {'status': StatusConstantData.objects.get(code='closed'),
				# 				   'preceptorship_activity': Preceptorship.objects.get(
			...
			return self.send_response(code=f'422',status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,description="Preceptorship doesn't exists")
		except FieldError:
			return self.send_response( code=f'500',description="Cannot resolve keyword given in 'order_by' into field")
		except Exception as e:
			return self.send_response(code=f'500',description=e)
	...
	def put(self, request, pk: None):
		try:
			data = request.data
			proctorship_attendance = InvoicePerceptorship.objects.get(preceptorship__id=pk)
			serializer = InvoicePerceptorshipUpdateSerializer(instance=proctorship_attendance, data=data)
			if serializer.is_valid():
				serializer.save()
				return self.send_response(success=True, code=f'201', status_code=status.HTTP_201_CREATED,
...
class AttendanceFormPerceptorshipView(BaseAPIView):
	...
	def get(self, request, pk=None):
		try:
			...
			if pk is not None:
				proctorship_attendance = AttendanceFormPerceptorship.objects.filter(preceptorship__id=pk)
				serializer = AttendancePerceptorShipSerailizers(proctorship_attendance, many=True)
				return self.send_response(success=True,status_code=status.HTTP_200_OK, payload=serializer.data,code= '',description='Details of serializer',log_description='')

			if activityid:
				proctorship_attendance = AttendanceFormPerceptorship.objects.filter(preceptorship__id=activityid)
				serializer = AttendancePerceptorShipSerailizers(proctorship_attendance, many=True)
				return self.send_response(success=True,status_code=status.HTTP_200_OK, payload=serializer.data,code= '',description='Details of serializer',log_description='')

			proctorship_attendance = AttendanceFormPerceptorship.objects.all()
			serializer = AttendancePerceptorShipSerailizers(proctorship_attendance, many=True)
			return self.send_response(success=True,status_code=status.HTTP_200_OK, payload=serializer.data,code= '',description='Details of serializer',log_description='', count = proctorship_attendance.count())
		except ObjectDoesNotExist:
			return self.send_response(code='422', status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, description="No ProctorShip Invoice matches the given query.")
	...
	def post(self, request):
		try:
			...
			if serializer.is_valid():
				...
				perceptership(Preceptorship.objects.get(id=data["preceptorship_id"]))
				# count = TraineePreceptorshipProfile.objects.filter(preceptorship__id=data["preceptorship_id"]).count()
				# if InvoicePerceptorship.objects.filter(preceptorship__id=data["preceptorship_id"]) and  PreceptorshipTraineeUploads.objects.filter(
				# 			preceptorship_trainee__preceptorship__id=data["preceptorship_id"]).count() == count:
				# 	PreceptorshipStatus.objects.filter(preceptorship_activity__id=data["preceptorship_id"]).update(
				# 		is_active=False)
				# 	status_data = {'status': StatusConstantData.objects.get(code='closed'),
	...
	def put(self, request, pk: None):
		try:
			data = request.data
			proctorship_attendance = AttendanceFormPerceptorship.objects.get(preceptorship__id=pk)
			serializer = AttendanceFormPerceptorshipUpdateSerailizers(instance=proctorship_attendance, data=data)
			if serializer.is_valid():
				serializer.save()
				return self.send_response(success=True, code=f'201', status_code=status.HTTP_201_CREATED,
										  description='File is replaced')
