...
from api.status.models import Proposal, Status, StatusConstantData
from api.status.serializers import (
    AlternativeSerializer,
    ConstantStatusDataSerializer,
    ProposalDateUpdateSerializer,
    StatusSerializer,
    StatusTestingSerializer,
...
class StatusView(BaseAPIView):
    ...
    def get(self, request, pk=None):
        try:
            ...
            if activity_id:
                activity_status = Status.objects.filter(
        ...
        try:
            ...
            start_date = str(
                Proposal.objects.filter(
                    status__proctorship_activity__id=data["proctorship_activity_id"]
                )
                .latest("created_on")
                .start_date
            )
            ...
            if data["status"] == "confirmed":
                proctors = (
                    Status.objects.filter(
                        proctorship_activity__id=data["proctorship_activity_id"]
                    )
                    .filter(alter_proctorship_porposal__isnull=False)
                    .latest("created_on")
                    .alter_proctorship_porposal.filter(proctor_porposal__isnull=False)
                    .latest("created_on")
                    .proctor_porposal.all()
                )
...
class AlternativePorposalView(BaseAPIView):
    ...
    def get(self, request, pk=None):
        try:
            activity_id = request.query_params.get("activity_id", "")
            activity_status = Proposal.objects.filter(
                proctorship_activity__id=activity_id
            )
        ...
        try:
            ...
            if serializer.is_valid():
                ...
                return self.send_response(
                    ...
                    code=f"201",
                    status_code=status.HTTP_201_CREATED,
                    description="Proposal is created",
                )
            else:
                return self.send_response(
                    code=f"422",
    ...
    def get(self, request, pk=None):
        try:
            ...
            return self.send_response(
                code=f"422",
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                description="Status Constant Data doesn't exists",
            )
        except FieldError:
            return self.send_response(
                code=f"500",
...
class DateUpdateView(BaseAPIView):
    ...
    def put(self, request, pk=None):
        try:
            ...
            serializer = ProposalDateUpdateSerializer(
