...
class AlternativeSerializer(serializers.ModelSerializer):
	...
	def get_proctors(self, obj):
		try:
			return ProctorshipProctorsSerializer(obj.proctor_porposal.proctors, many=True).data
		except:
			return None
...
class AlternativeViewSerializer(serializers.ModelSerializer):
	...
	def get_proctor(self, obj):
		try:
			return ProctorshipProctorsSerializer(obj.proctor_porposal.all(), many=True).data
		except:
			return None
...
class StatusSerializer(serializers.ModelSerializer):
	...
	class Meta:
		model = Status
		fields = ['id','user','status', 'user_id', 'reason','proctorship_activity_id','code', 'date', 'purposal', 'alternatives_data']
	...
	def create(self, validated_data):
		try:
			with transaction.atomic():
				...
				Status.objects.filter(proctorship_activity__id=proctorship_activity_id).update(is_active=False)
				...
				if len(alternatives_data) != 0:
					...
					if 'proctor_user_id' in alternatives_data.keys():
						qs = ProctorshipProctors.objects.filter(porposal__status__proctorship_activity__id=proctorship_activity_id, status = True)
