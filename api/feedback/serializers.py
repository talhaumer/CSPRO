    ...
    def create(self, validated_data):
        try:
            with transaction.atomic():
                ...
                feedback.rate_of_experince = Rating.objects.get(
                    code=rate_of_experince_code
                )
                feedback.save()
                return feedback
...
class MemoProctorUpdateFeedbackSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        try:
            ...
            instance.implanted_memo = validated_data.get(
                "implanted_memo", instance.implanted_memo
            )
            instance.is_trainee_implant = validated_data.get(
                "is_trainee_implant", instance.is_trainee_implant
            )
            ...
            instance.report = validated_data.get("report", instance.report)
            if "rate_of_experince_code" in validated_data.keys():
                validated_data["rate_of_experince_code"] = Rating.objects.get(
                    code=validated_data.pop("rate_of_experince_code")
                )
                instance.rate_of_experince = validated_data.get(
                    "rate_of_experince", instance.rate_of_experince
                )
    ...
    def create(self, validated_data):
        try:
            with transaction.atomic():
                ...
                rate_of_experince_code = validated_data.pop("rate_of_experince_code")
                feedback = TraineeFeedback.objects.create(**validated_data)
                feedback.rate_of_training = Rating.objects.get(
...
class TraineeMemoUpdateFeedbackSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        try:
            ...
            instance.report = validated_data.get("report", instance.report)
            instance.is_memo_family = validated_data.get(
                "is_memo_family", instance.is_memo_family
            )
            if "rate_of_experince_code" in validated_data.keys():
                validated_data["rate_of_experince"] = Rating.objects.get(
                    code=validated_data.pop("rate_of_experince_code")
            ...
            if "rate_of_training_code" in validated_data.keys():
                validated_data["rate_of_training"] = Rating.objects.get(
                    code=validated_data.get("rate_of_training_code")
                )
...
class SoloSmartUpdateFeedbackSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        instance.implanted_solo_smart = validated_data.get(
            "implanted_solo_smart", instance.implanted_solo_smart
        )
        instance.is_trainee_implant = validated_data.get(
            "is_trainee_implant", instance.is_trainee_implant
        )
