...
from api.masterproctorship.models import MasterProctorshipProposal, MasterProctorshipProctors, MasterProctorship
...
from api.speakingevent.models import Event, SpeakingEvent, Speaker
from api.status.models import ProctorshipProctors, Proposal
from api.users.models import User
from api.views import BaseAPIView
from api.proctors.models import Proctors, query_proctors_by_args
from api.proctors.serializers import ProctorsZoneViewSerializer, ProctorsViewSerializer, ProctorsUpdateSerializer, \
    ProctorsSerializer, ProctorsZoneDropViewSerializer
from api.permissions import IsGETOAuthenticatedSalesManager, \
    IsGetOrIsAuthenticated, IsGetOAuthenticatedSuperAdminLocalAdminSalesManager, IsPostOAuthenticatedSuperAdminUpdate, \
    IsOauthAuthenticatedSuperAdminLocalAdminPostOrGet, IsOauthAuthenticatedSuperAdminLocalAdmin, IsOauthAuthenticated
...
class ProctorsOfZoneView(BaseAPIView, CustomPagination):
    ...
    def get(self, request, pk=None):
        try:
            ...
            is_master_proctorshhip = boolean(request.query_params.get('is_master_proctorshhip', ''))
            ...
            if is_zone and event:
                ...
                proctors = Proctors.objects.filter(user__is_active=True, contract_ending_details__gt=end_date,
                                                       user__country__id__in=countries_id)

                proctors = proctors.filter(query_object)
                if activity_id:
                    speakers = Speaker.objects.filter(event__id=activity_id, proctor__isnull=False,
                                                  revoke=False)
                ...
                if search_value:
                    proctors = proctors.filter(Q(user__name__icontains=search_value)).distinct()

                if limit:
                    serializer = ProctorsZoneViewSerializer(proctors[offset: offset + int(limit)], many=True)
                else:
            ...
            if is_global and event:
                ...
                proctors = proctors.exclude(unavailability_start_date__lt=start_date,
            ...
            if is_zone:
                countries = request.user.admin_user.get().zone.zone_zone_countires.all()
                countries_id = [country.countries.id for country in countries]
                proctors = Proctors.objects.filter(user__is_active = True, only_speaker=False,products__id=product, user__country__id__in = countries_id,contract_ending_details__gt=end_date)

                proctors = proctors.exclude( unavailability_start_date__lt=start_date, unavailability_end_date__gt=start_date)
                proctors = proctors.exclude(unavailability_start_date__gte=end_date,
                                            unavailability_end_date__lte=end_date)
                proctors  = available_proctors(proctors, start_date, end_date)
            ...
            if is_global:
                proctors = Proctors.objects.filter(user__is_active = True, only_speaker=False, products__id=product,
            ...
            if is_master_proctorshhip:
                proctors = Proctors.objects.filter(user__is_active = True, only_speaker=False, is_masterproctorship=True)
            ...
            return self.send_response(success=True, status_code=status.HTTP_200_OK, payload=[],
                                      code='', description='Details of serializer', log_description="")
...
class GloabalProctorsView(BaseAPIView, CustomPagination):
    ...
    def get(self, request, pk=None):
        try:
            products_id = request.query_params.get('products_id', '')
            activity_date = request.query_params.get('activity_date', '')
            # proctors = Proctors.objects.filter(contract_starting_details__lte=activity_date, contract_ending_details__gte=activity_date)
            # proctors = proctors.exclude(unavailability_start_date__lte=activity_date, unavailability_end_date__gte=activity_date)
            proctors = Proctors.objects.all()
...
#       try:
#           if pk is not None:
#               proctor = Proctors.objects.get(id=pk, deleted = False)
#               proctor.deleted = True
#               print(proctor)
#               return self.send_response(description= "Proctors is deleted successfuly",success=True,status_code=status.HTTP_200_OK, code='200')
#           return self.send_response(code=f'422',status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, description="Proctors doesn't found")
#       except Proctors.DoesNotExist:
...
# class ProctorsGetView(BaseAPIView):
...
#           proctors = Proctors.objects.all()
#           serializer = ProctorsViewSerializer(proctors, many=True)
#           return self.send_response(success=True,status_code=status.HTTP_200_OK, payload=serializer.data,code= '',description='Details of serializer',log_description='', count = len(proctors))
#       except ObjectDoesNotExist:
...
#       except Proctors.DoesNotExist:
#           return self.send_response(code='', status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, description="Proctors doesn't exists")
#       except FieldError:
#           return self.send_response(code=f'500', description="Cannot resolve keyword given in 'order_by' into field")
#       except Exception as e:
#           return self.send_response( code=f'500',description=e)
...
# class ProctorsViewSet(viewsets.ModelViewSet, BaseAPIView):
#   permission_classes = (IsGetOrIsAuthenticated, )
#   queryset = Proctors.objects.all()
#   serializer_class = ProctorsSerializer
...
#           area_of_experties = request.query_params.get('area_of_experties_id', '')
#           spoken_languages = request.query_params.get('spoken_languages_id', '')
#           approach = request.query_params.get('approach_id', '')
#           products = request.query_params.get('products_id', '')
#           only_speaker = request.query_params.get('only_speaker', '')
#           Only_Speaker = {'Yes':True, 'No':False}

#           query_object = Q()

#           if country != '':
#               query_object &= Q(country__id = country)

#           if hospital != '':
#               query_object &= Q(hospital__id = hospital)

#           if area_of_experties != '':
#               query_object = Q(area_of_experties__id = area_of_experties)

#           if spoken_languages != '':
#               query_object = Q(spoken_languages__id = spoken_languages)

#           if approach != '':
#               query_object = Q(approach__id = approach)

#           if products != '':
#               query_object = Q(products__id = products)

#           if only_speaker != '':
#               query_object &= Q(only_speaker = Only_Speaker[only_speaker])


#           proctors = query_proctors_by_args(query_object, **request.query_params)

#           serializer = ProctorsSerializer(proctors['items'], many=True)
#           print(serializer.data)

#           result = dict()
#           result['data'] = serializer.data
#           result['draw'] = proctors['draw']
#           result['recordsTotal'] = proctors['total']
#           result['recordsFiltered'] = proctors['count']
...
class ProctorsPaginationView(BaseAPIView, CustomPagination):
    ...
    def get(self, request, pk=None):
        try:
            ...
            area_of_experties = request.query_params.get('area_of_experties', None)
            spoken_languages = request.query_params.get('spoken_languages', None)
            approach = request.query_params.get('approach', None)
            ...
            proctors = Proctors.objects.all()
            if search_value:
                proctors = proctors.filter(Q(id__icontains=search_value) |
                                           ...
                                           Q(approach__name__icontains=search_value)).distinct()
            ...
            if is_dashboard:
                expired_soon = proctors.filter(contract_ending_details__gt=datetime.datetime.now()).order_by(
                    "contract_ending_details")[0:10]
                serializer = ProctorsViewSerializer(expired_soon, many=True)
                expired = proctors.filter(contract_ending_details__lt=datetime.datetime.now()).order_by(
                    "-contract_ending_details")[0:10]
                expired = ProctorsViewSerializer(expired, many=True)
                res = {"expired soon":serializer.data,
                       "expired":expired.data}
                return self.send_response(success=True, status_code=status.HTTP_200_OK, payload=res,
                                          code='',
                                          description='Details of serializer', log_description='',
                                          count=proctors.count())

            serializer = ProctorsViewSerializer(proctors[offset: offset + limit], many=True)
...
class ProctorsOfCOEView(BaseAPIView, CustomPagination):
    ...
    def get(self, request, pk=None):
        try:
            ...
            proctors = Proctors.objects.filter(proctors_pivot_id__hospital=hospital, only_speaker=False,user__is_active = True)
            proctors = proctors.exclude(proctorShip_contract_ending_details__lt = end_date)
    ...
    def get(self, request, pk=None):
        try:
            if pk is not None:
                ...
                proctor.save()
                return self.send_response(success=True, status_code=status.HTTP_200_OK, payload=[], code='',
                                          description='Deleted Successfully', log_description='')
        except ObjectDoesNotExist as e:
            return self.send_response(code='422', status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                                      description="No Proctor matches the given query.")
        except Proctors.DoesNotExist:
...
class ProctorsTestView(BaseAPIView, CustomPagination):
    ...
    def get(self, request, pk=None):
        try:
            ...
            proctors = Proctors.objects.filter(products__id=product, only_speaker = False,user__is_active = True,contract_starting_details__lt=start_date,
                                               contract_ending_details__gt = end_date)

            proctors = proctors.exclude(unavailability_start_date__lt=start_date, unavailability_end_date__gt=end_date)

            proctors = proctors.exclude(porposal_proctor_id__porposal__status__status__code__in=["waiting-for-docs", "confirmed"],
                                        porposal_proctor_id__status=True, porposal_proctor_id__porposal__status__is_active=True,
                                        porposal_proctor_id__porposal__start_date__lt=start_date,porposal_proctor_id__porposal__end_date__gt=end_date)
