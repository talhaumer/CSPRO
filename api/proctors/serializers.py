...
class ProctorsSerializer(serializers.ModelSerializer):
    ...
    proctorShip_contract_start_details = serializers.DateField(
        write_only=True, allow_null=True
    )
    proctorShip_contract_ending_details = serializers.DateField(
        write_only=True, allow_null=True
    )
    ...
    reason_why = serializers.CharField(
        write_only=True, allow_blank=True, allow_null=True
    )
    ...
    area_of_experties_id = serializers.CharField(
        write_only=True, allow_null=True, allow_blank=True
    )
    approach_id = serializers.CharField(
        write_only=True, allow_null=True, allow_blank=True
    )
    spoken_languages_id = serializers.CharField(
        write_only=True, allow_null=True, allow_blank=True
    )
        ...
        try:
            with transaction.atomic():
                proctor_data = {
                    ...
                    "only_speaker": validated_data.pop("only_speaker"),
                    "contract_starting_details": validated_data.pop(
                        "contract_starting_details"
                    ),
                    "contract_ending_details": validated_data.pop(
                        "contract_ending_details"
                    ),
                    "resume": validated_data.pop("resume"),
                    "proctorShip_contract_start_details": validated_data.pop(
                        "proctorShip_contract_start_details"
                    ),
                    "proctorShip_contract_ending_details": validated_data.pop(
                        "proctorShip_contract_ending_details"
                    ),
                    "unavailability_start_date": validated_data.pop(
                        "unavailability_start_date"
                    ),
                    "unavailability_end_date": validated_data.pop(
                        "unavailability_end_date"
                    ),
                    "reason_why": validated_data.pop("reason_why"),
                    "is_masterproctorship": validated_data.pop("is_masterproctorship"),
                }
    ...
    def update(self, instance, validated_data):
        ...
        if "country_id" in validated_data.keys():
            validated_data["country"] = Countries.objects.get(
                pk=int(validated_data.pop("country_id"))
            )
        ...
        instance.only_speaker = validated_data.get(
            "only_speaker", instance.only_speaker
        )
        instance.contract_starting_details = validated_data.get(
            "contract_starting_details", instance.contract_starting_details
        )
        instance.contract_ending_details = validated_data.get(
            "contract_ending_details", instance.contract_ending_details
        )
        if "resume" in validated_data.keys():
            instance.resume = validated_data.get("resume", instance.resume)
        instance.proctorShip_contract_start_details = validated_data.get(
            "proctorShip_contract_start_details",
            instance.proctorShip_contract_start_details,
        )
        instance.proctorShip_contract_ending_details = validated_data.get(
            "proctorShip_contract_ending_details",
            instance.proctorShip_contract_ending_details,
        )
        instance.unavailability_start_date = validated_data.get(
            "unavailability_start_date", instance.unavailability_start_date
        )
        instance.unavailability_end_date = validated_data.get(
            "unavailability_end_date", instance.unavailability_end_date
        )
        ...
        if "hospital_id" in validated_data.keys():
            ...
            new_hospital = ProctorsHospital.objects.create(
                proctors=instance, hospital=hospital, status=True
            )
            new_hospital.save()
        instance.is_masterproctorship = validated_data.get(
            "is_masterproctorship", instance.is_masterproctorship
        )
