...
from api.masterproctorship.models import MasterProctorshipProctorReport, MasterProctorship, \
    MasterProctorshipTraineeProfile, MasterProctorshipProctors, MasterProctorshipProposal, MasterProctorshipStatus, \
    MasterProctorShipConstantData, MasterProctorshipFeedback, InvoiceMasterProctorShip, AttendanceFormMasterProctorShip
from api.serializers import Products, Hospital
...
class MasterProctorshipSerializer(serializers.ModelSerializer):
    ...
    master_proctorship_type = serializers.CharField(required=True, allow_null=True, allow_blank=True)
    note_related_to_proctor = serializers.CharField(required=False, allow_null=True, allow_blank=True)
    hotel = serializers.CharField(required=True)
    number_of_cases = serializers.IntegerField(required=True, allow_null=True)
    transplant_time = serializers.CharField(required=True, allow_blank=True, allow_null=True)
    ...
    class Meta:
        ...
        fields = ['id', 'user', 'country', 'hospital', 'note', 'reason', 'master_proctorship_type',
                  'note_related_to_proctor', 'hotel', 'number_of_cases', 'transplant_time', 'start_date', 'end_date',
                  'user_id', 'country_id', 'hospital_id', 'proctors_id', 'trainee_profile']
        ...
        try:
            with transaction.atomic():
                ...
                if master_proctorship_type:
                    masterproctorship.master_proctorship_type = MasterProctorShipConstantData.objects.get(
...
# def get_proctors(obj):
# 	try:
# 		return MasterProctorshipProctorsSerializer(obj.master_proctorship_porposal.proctors, many=True).data
# 	except:
# 		return None
...
class MasterProctorshipProposalViewSerializer(serializers.ModelSerializer):
    ...
    def get_proctor(self, obj):
        try:
            return MasterProctorshipProctorsSerializer(obj.master_proctorship_porposal.all(), many=True).data
        except:
            return None
...
class MasterProctorshipStatusSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'user', 'status', 'user_id', 'reason', 'master_proctorship_activity_id', 'code', 'date',
    ...
    def create(self, validated_data):
        try:
            with transaction.atomic():
                ...
                master_proctorship_activity_id = validated_data.pop('master_proctorship_activity_id')
                validated_data['master_proctorship_activity_id'] = master_proctorship_activity_id

                MasterProctorshipStatus.objects.filter(master_proctorship_activity__id=master_proctorship_activity_id).update(is_active=False)
                ...
                if len(alternatives_data) != 0:
                    ...
                    if 'proctor_user' in alternatives_data.keys():
                        ...
                        proctors = {
                            ...
                            'proctors': Proctors.objects.get(id=alternatives_data['proctor_user']),
                            'proctor_order': 1
                        }
    ...
    def get_alternatives(self, obj):
        try:
            return MasterProctorshipProposalViewSerializer(obj.alter_proctorship_porposal.get()).data
        except:
            return None
...
class MasterProctorshipStatusViewSerializer(serializers.ModelSerializer):
    ...
    def get_alternatives(self, obj):
        try:
            return MasterProctorshipProposalViewSerializer(obj.alter_master_proctorship_porposal.get()).data
        except:
            return None
...
class MasterProctorshipViewSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = MasterProctorship
        fields = ['id', 'user', 'country', 'hospital', 'note', 'reason', 'master_proctorship_type', 'hotel',
                  'number_of_cases', 'transplant_time', 'status', 'hospital_id', 'country_id', "activity_id"]
    ...
    def get_status(self, obj):
        try:
            return MasterProctorshipStatusViewSerializer(obj.master_proctorship_status.all(), many=True).data
        except:
            return None
...
class TraineeMasterProctorshipSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'status', 'master_proctorship_id', 'interest_invasive', 'revoke', 'hospital', 'hospital_id',
                  'country',
    ...
    def create(self, validated_data):
        try:
            with transaction.atomic():
                ...
                trainee = MasterProctorshipTraineeProfile.objects.create(**validated_data)
...
class TraineeMasterUpdateProctorshipSerializer(serializers.ModelSerializer):
	...
	class Meta:
		model = MasterProctorshipTraineeProfile
		fields = ['id','status', 'interest_invasive', 'revoke', 'title', 'name', 'surname', 'corcym_accompanying_rep', 'current_preferential',
				  'mvr_case_per_year', 'mvr_case_per_year_by_trainee', 'note']
...
class MasterProctorshipProctorReportSerializers(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["master_proctorship_trainee_id",
                  "num_of_patients",
                  "num_of_perceval",
                  "rate_of_experince",
                  "have_difficulties",
    ...
    def create(self, validated_data):
        try:
            with transaction.atomic():
                ...
                proctor_report = MasterProctorshipProctorReport.objects.create(**validated_data)
                proctor_report.rate_of_experince = Rating.objects.get(code=rate_of_experince)
                proctor_report.save()
...
class MasterProctorshipProctorUpdateReportSerializers(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        try:
            ...
            instance.num_of_perceval = validated_data.get('num_of_perceval', instance.num_of_perceval)
            instance.rate_of_experince = validated_data.get('rate_of_experince', instance.rate_of_experince)
            instance.have_difficulties = validated_data.get('have_difficulties', instance.have_difficulties)
            instance.need_for_further_training = validated_data.get('need_for_further_training', instance.need_for_further_training)
...
class MasterProctorshipFeedbackSerializers(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['master_proctorship_activity_id','num_of_patients','rate_of_proceduresc','rate_proctoring_experince','rate_of_level_support','new_proctor_need_further_training','suggestion','report']

    def create(self, validated_data):
        try:
            with transaction.atomic():
                rate_of_level_support_code = validated_data.pop("rate_of_level_support")
                rate_proctoring_experince_code = validated_data.pop('rate_proctoring_experince')
                feedback = MasterProctorshipFeedback.objects.create(**validated_data)
                feedback.rate_of_level_support = Rating.objects.get(code = rate_of_level_support_code)
                feedback.rate_proctoring_experince= Rating.objects.get(code = rate_proctoring_experince_code)
                feedback.save()
                return feedback
        except Exception as e:
            return e
...
class MasterProctorshipUpdateFeedbackSerializers(serializers.ModelSerializer):
    ...
    class Meta:
        model = MasterProctorshipFeedback
        fields = ['master_proctorship_activity','num_of_patients','rate_of_proceduresc','rate_proctoring_experince','rate_of_level_support','new_proctor_need_further_training','suggestion','report']

    def update(self, instance, validated_data):
        try:
            if 'rate_of_level_support' in validated_data.keys():
                validated_data['rate_of_level_support']= Rating.objects.get(code = validated_data.pop("rate_of_level_support"))
                instance.rate_of_level_support = validated_data.get('rate_of_level_support', instance.rate_of_level_support)

            if 'rate_proctoring_experince' in validated_data.keys():
                validated_data['rate_proctoring_experince']= Rating.objects.get(code = validated_data.pop("rate_proctoring_experince"))
                instance.rate_proctoring_experince = validated_data.get('rate_proctoring_experince', instance.rate_proctoring_experince)

            instance.num_of_patients = validated_data.get('num_of_patients',
                                                                    instance.num_of_patients)
            instance.rate_of_proceduresc = validated_data.get('rate_of_proceduresc',
                                                                    instance.rate_of_proceduresc)
            instance.new_proctor_need_further_training = validated_data.get('new_proctor_need_further_training',
...
class MasterProctorshipListingSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = MasterProctorship
        fields = ['id', 'user', 'country', 'hospital', 'note', 'reason', 'master_proctorship_type', 'hotel',
                  'number_of_cases', 'transplant_time', 'hospital_id', 'country_id', 'status', 'proctor',
                  'date', "activity_id", "cognos_id"]
    ...
    def get_status(self, obj):
        try:
            return MasterProctorshipStatusViewSerializer(obj.master_proctorship_status.latest('timestamp')).data['code']
        except:
            return None
    ...
    def get_proctor(self, obj):
        try:
            ...
            return MasterProctorshipProctorsSerializer(obj.master_proctorship_status.filter(alter_master_proctorship_porposal__isnull=False).latest(
                'created_on').alter_master_proctorship_porposal.filter(master_proctorship_porposal__isnull=False).latest(
                'created_on').master_proctorship_porposal.filter(status = True), many=True).data
        except:
            return None
    ...
    def get_date(self, obj):
        try:
            ...
            return obj.master_proctorship_status.filter(alter_master_proctorship_porposal__isnull=False).latest(
                'created_on').alter_master_proctorship_porposal.get().start_date
        except:
            return None
...
class InvoiceMasterProctorshipSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['master_proctorship_id', 'invoice_number', 'fee_covered', 'other_cost', 'invoice_date', 'note', 'invoice_sent']
...
class AttendanceFormMasterProctorShipSerailizers(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        try:
            with transaction.atomic():
                attendace_form = AttendanceFormMasterProctorShip.objects.create(**validated_data)
                attendace_form.save()
...
class MasterProctorshipStatusTestingSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = MasterProctorshipStatus
        fields = ['id','status', 'reason','master_proctorship_activity_id','code', 'date']
