...
from api.permissions import IsOauthAuthenticated, IsOauthAuthenticatedSuperAdmin
from api.views import BaseAPIView
from cspro.utilities.convert_boolean import boolean
from cspro.utils import send_email_sendgrid_template

from .models import EmailToken, User, UserInfo, query_user_by_args
from .serializers import (
    ChangePasswordSerializer,
    PasswordSerializers,
    UpdateProfileDataSerializer,
        ...
        try:
            ...
            if serializer.is_valid():
                ...
                EmailToken.objects.create(
                    user=serializer.instance, token=password_token
                )

                link = f"{settings.WEB_URL}activate-account/{str(password_token)}"

                data = {"first_name": serializer.data["name"], "Link": link}
        ...
        try:
            ...
            if obj:
                ...
                if serializer.is_valid():
                    ...
                    user.is_active = True
                    user.save()
                    deleted = EmailToken.objects.filter(
                        user__id=obj.user_id, token__exact=data["token"]
                    ).delete()
                    return self.send_response(
                        success=True,
                        code=f"201",
...
class GetUserView(BaseAPIView, CustomPagination):
    ...
    def filtration(
        self, zone, name, country, email, is_active, role, mail_notification
    ):
        try:
            query_object = Q()
    ...
    def get(self, request, pk=None):
        try:
            ...
            query_object = self.filtration(
                zone, name, country, email, is_active, role, mail_notification
            )
    ...
    def get(self, request, pk=None):
        ...
        try:
            ...
            return self.send_response(
                ...
                payload=serializer.data,
                status_code=status.HTTP_200_OK,
                description="User Sales Manager",
                count=user.count(),
            )
        except User.DoesNotExist:
            return self.send_response(
        ...
        try:
            if pk is not None:
                ...
                if user.is_active == True:
                    return self.send_response(
                        description="users is activate successfuly",
                        success=True,
                        status_code=status.HTTP_200_OK,
                        code="200",
                    )
                    ...
                    return self.send_response(
                        description="users is Blocked successfuly",
                        success=True,
                        status_code=status.HTTP_200_OK,
                        code="200",
                    )
...
class UpdateUserFormView(BaseAPIView):
    ...
    def put(self, request, pk=None):
        try:
            ...
            data = request.data
            serializer = UserFormUpdateSerializer(
                instance=saved_user, data=data, partial=True
            )
            if serializer.is_valid():
    ...
    def get(self, request, pk=None):
        try:
            if pk is not None:
                ...
                print(user)
                return self.send_response(
                    description="users is deleted successfuly",
                    success=True,
                    status_code=status.HTTP_200_OK,
                    code="200",
                )
...
class UserPaginationView(BaseAPIView, CustomPagination):
    ...
    def get(self, request, pk=None):
        try:
            ...
            if rev == "true":
                ordered = sorted(
                    users, reverse=True, key=operator.attrgetter(column_name)
                )
            else:
                ordered = sorted(
                    users, reverse=False, key=operator.attrgetter(column_name)
                )
