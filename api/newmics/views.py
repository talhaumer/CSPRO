...
from api.newmics.serializer import (
    CEOProctorsZoneViewSerializer,
    HospitalForMICSSerializer,
    MicsAttendanceFormSerailizers,
    MicsCertificateFormSerailizers,
    MicsInvoiceSerializer,
    MicsPercevalSerializer,
    MicsPreceptorshipStatusSerializer,
    MicsProctorshipSerializer,
    MicsProctorshipStatusSerializer,
    MicsSerializer,
    MicsTraineeFeedbackSerializer,
    MicsTraineeProfileSerializer,
    MicsViewListingSerializer,
    MicsViewSerializer,
    RecentPreceptorshipViewSerializer,
    TraineeMicsViewSerializer,
    UpdateStatusPerceptershipSerializer,
    UpdateStatusProctorshipSerializer,
)
        ...
        try:
            ...
            if serializer.is_valid():
                ...
                return self.send_response(
                    ...
                    description="New Mics is created",
                )
            return self.send_response(
                code=f"422",
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
...
class MicsTraineeProfileView(BaseAPIView):
    ...
    def post(self, request, pk=None):
        try:
            mics = NewMics.objects.get(id=pk)
            data = request.data
            serializer = MicsTraineeProfileSerializer(
                data=data, many=True, context={"mics": mics}
            )
            if serializer.is_valid():
                serializer.save()
                return self.send_response(
...
class TraineeMicsView(BaseAPIView):
    ...
    def get(self, request, pk=None):
        try:
            ...
            if mics_id:
                trainees = MicsTraineeProfile.objects.filter(
                    mics__id=mics_id,
                )
            ...
            return self.send_response(
                ...
                payload=[],
                code="422",
                description="Enter valid Mics Id",
                log_description="",
            )
        except ObjectDoesNotExist:
            return self.send_response(
...
class MicsProctorshipStatusView(BaseAPIView):
    ...
    def get(self, request, pk=None):
        try:
            ...
            if activity_id:
                activity_status = MicsProctorshipStatus.objects.filter(
                    proctorship_activity__id=activity_id
                ).order_by("timestamp")
            ...
            return self.send_response(
                ...
                payload=[],
                code="422",
                description="Enter Valid Mics ProctorshipId",
            )
        ...
        try:
            try:
                ...
                mics_per = MicsProctorshipProposal.objects.filter(
                    status__proctorship_activity__mics__id=pk
                ).latest("created_on")
                ...
                if data["status"] == "confirmed":
                    proctors = (
                        MicsProctorshipStatus.objects.filter(
                            proctorship_activity__mics__id=pk
                        )
                        .filter(mics_proctorship_porposal__isnull=False)
                        .latest("created_on")
                        .mics_proctorship_porposal.filter(
                            mics_proctor_porposal__isnull=False
                        )
                        .latest("created_on")
                        .mics_proctor_porposal.all()
                    )
                            ...
                            if not check_perceptorship(
                                ...
                                if not check_masterproctorship(
                                    ...
                                    if not check_event(
                                        ...
                                        each.status = True
                                        each.save()
                                        serializer = UpdateStatusProctorshipSerializer(
                                            data=data, context={"mics": mics}
                                        )
                                        if serializer.is_valid():
                                            validated_data = serializer.validated_data
                                            validated_data["user"] = request.user
        ...
        try:
            ...
            if activity_id:
                activity_status = MicsPreceptorshipStatus.objects.filter(
                    mics_preceptorship_activity__id=activity_id
                ).order_by("timestamp")
                serializer = MicsPreceptorshipStatusSerializer(
            ...
            return self.send_response(
                ...
                payload=[],
                code="422",
                description="Enter Valid Mics Perceptorship id",
            )
        except IntegrityError as i:
            return self.send_response(
                code=f"422",
    ...
    def post(self, request, pk=None):
        try:
            ...
            mics_per = MicsPreceptorshipProposal.objects.filter(
                status__mics_preceptorship_activity__mics__id=pk
            ).latest("created_on")
            ...
            if data["status"] == "confirmed":
                proctors = (
                    MicsPreceptorshipStatus.objects.filter(
                        mics_preceptorship_activity__mics__id=pk
                    )
                    .filter(alter_mics_preceptorship_porposal__isnull=False)
                    .latest("created_on")
                    .alter_mics_preceptorship_porposal.filter(
                        mics_preceptorship_porposal__isnull=False
                    )
                    .latest("created_on")
                    .mics_preceptorship_porposal.all()
                )
                for each in proctors:
                    if not check(each.proctors.id, start_date, end_date):
                        if not check_perceptorship(
                            ...
                            if not check_masterproctorship(
                                ...
                                if not check_event(
                                    ...
                                    each.status = True
                                    each.save()
                                    serializer = UpdateStatusPerceptershipSerializer(
                                        data=data, context={"mics": mics}
                                    )
                                    if serializer.is_valid():
                                        validated_data = serializer.validated_data
...
class NewMicsCourseListing(BaseAPIView, CustomPagination):
    ...
    def get(self, request, pk=None):
        try:
            ...
            new_mics = NewMics.objects.all()
            if search_value:
                new_mics = new_mics.filter(
                    Q(id__icontains=search_value)
                    | Q(user__name__icontains=search_value)
                    | Q(
                        mics_perceptotship__hospital__hospital_name__icontains=search_value
                    )
                    | Q(
                        mics_perceptotship__product__product_name__icontains=search_value
                    )
                    | Q(
                        mics_perceptotship__mics_preceptorshipStatus_status__status__code__icontains=search_value
                    )
                    | Q(
                        mics_perceptotship__mics_preceptorshipStatus_status__alter_mics_preceptorship_porposal__start_date__icontains=search_value
                    )
                    | Q(
                        mics_perceptotship__mics_preceptorshipStatus_status__alter_mics_preceptorship_porposal__mics_preceptorship_porposal__proctors__user__name__icontains=search_value
                    )
                    | Q(mics_peroctorship__country__name__icontains=search_value)
                    | Q(
                        mics_peroctorship__hospital__hospital_name__icontains=search_value
                    )
                    | Q(mics_peroctorship__hotel__icontains=search_value)
                    | Q(mics_peroctorship__number_of_cases__icontains=search_value)
                    | Q(
                        mics_peroctorship__mics_proctorship_status__status__code__icontains=search_value
                    )
                    | Q(
                        mics_peroctorship__mics_proctorship_status__mics_proctorship_porposal__start_date__icontains=search_value
                    )
                    | Q(mics_peroctorship__transplant_time__icontains=search_value)
                    | Q(
                        mics_peroctorship__mics_proctorship_status__mics_proctorship_porposal__mics_proctor_porposal__proctors__user__name__icontains=search_value
                    )
                ).distinct()

            query_object = Q(
                mics_perceptotship__mics_preceptorshipStatus_status__is_active=True
            ) | Q(mics_peroctorship__mics_proctorship_status__is_active=True)
            ...
            new_mics.filter(query_object).order_by(
                str(self.sorting_data(column_name, order))
            )

            if actvity_status:
                query_object &= Q(
                    mics_peroctorship__mics_proctorship_status__status__code=actvity_status
                ) | Q(
                    mics_perceptotship__mics_preceptorshipStatus_status__status__code=actvity_status
                )

            new_mics = (
                new_mics.filter(query_object)
                .order_by(str(self.sorting_data(column_name, order)))
                .distinct()
            )

            serializer = MicsViewListingSerializer(
                new_mics[offset : offset + limit], many=True
            )
            return self.send_response(
                success=True,
                status_code=status.HTTP_200_OK,
    ...
    def get(self, request, pk=None):
        try:
            ...
            query_object = Q(
                mics__is_rat=is_rat,
                product__id=2,
                mics_preceptorshipStatus_status__status__code__in=[
                    "confirmed",
                    "processing",
                    "pending",
...
class MicsHospitalZoneView(BaseAPIView):
    ...
    def get(self, request, pk=None):
        try:
            ...
            query_object = Q(
                is_it_preceptorship=True, qualified_for_news_mics_program=True
            )
            if product:
    ...
    def get(self, request, pk=None):
        try:
            ...
            proctors = Proctors.objects.filter(
                query_object, only_speaker=False, user__is_active=True
            )
...
class MicsPrecevalFeedbackView(BaseAPIView):
    ...
    def post(self, request, name=None, pk=None):
        try:
            ...
            return self.send_response(
                code=f"422",
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                description="Feedback doesn't exists",
            )
        except MicsPreceptorship.DoesNotExist:
            return self.send_response(
                code=f"422",
    ...
    def put(self, request, name=None, pk=None):
        try:
            ...
            data = request.data
            serializer = MicsPercevalSerializer(
                instance=saved_product, data=data, partial=True
            )
            if serializer.is_valid():
            ...
            return self.send_response(
                code="422",
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                description="No Feedback matches the given query.",
            )
        except MicsPercevalFeedback.DoesNotExist:
            return self.send_response(
                code=f"422",
...
class MicsTraineeFeedbackView(BaseAPIView):
    ...
    def post(self, request, name=None, pk=None):
        try:
            ...
            if name == "perceptership":
                ...
                if (
                    obj.mics_trainee_feedback_mics_perceptership.filter(
                ...
                obj = MicsProctorship.objects.get(id=pk)
                if (
                    obj.mics_trainee_feedback_mics_proctorship.filter(
                        trainee_id=data["trainee_id"]
                    ).count()
                    > 0
                ):
            ...
            return self.send_response(
                code=f"422",
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                description="Trainee Feedback doesn't exists",
            )
        except MicsPreceptorship.DoesNotExist:
            return self.send_response(
                code=f"422",
    ...
    def put(self, request, name=None, pk=None):
        try:
            ...
            data = request.data
            serializer = MicsTraineeFeedbackSerializer(
                instance=saved_product, data=data, partial=True
            )
            if serializer.is_valid():
                product_saved = serializer.save()
            ...
            return self.send_response(
                code="422",
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                description="No Trainee Feedback matches the given query.",
            )
        except MicsTraineeFeedback.DoesNotExist:
            return self.send_response(
                code=f"422",
...
class MicsAttendanceFormView(BaseAPIView):
    ...
    def put(self, request, name=None, pk=None):
        try:
            ...
            serializer = MicsAttendanceFormSerailizers(
...
class MicsCertificateFormView(BaseAPIView):
    ...
    def put(self, request, name=None, pk=None):
        try:
            ...
            serializer = MicsCertificateFormSerailizers(
                instance=proctorship_attendance, data=data
            )
            if serializer.is_valid():
                serializer.save()
