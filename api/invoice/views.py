...
from api.invoice.serializer import InvoiceSerializer, AttendanceFormSerailizers, AttendanceFormUpdateSerailizers, \
...
class InvoiceView(BaseAPIView):
	...
	def get(self, request, pk=None):
		try:
			...
			return self.send_response(code='422', status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, description="No  Invoice matches the given query.")
		except Invoice.DoesNotExist:
			return self.send_response(code='', status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, description="Invoice doesn't exists")
		except FieldError:
	...
	def post(self, request):
		try:
			...
			if serializer.is_valid():
				...
				# if check_product_feedback(data["proctorship_id"]) and  count == TraineeFeedback.objects.filter(trainee__proctorship__id=data["proctorship_id"]).count() and AttendanceForm.objects.filter(proctorship__id=data["proctorship_id"]):
			...
			return self.send_response(code=f'422',status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,description="Invoice doesn't exists")
		except FieldError:
			return self.send_response( code=f'500',description="Cannot resolve keyword given in 'order_by' into field")
		except Exception as e:
			return self.send_response(code=f'500',description=e)
	...
	def put(self, request, pk:None):
		try:
			...
			if serializer.is_valid():
				serializer.save()
				return self.send_response(success=True, code=f'201', status_code=status.HTTP_201_CREATED, description='Invoice is replaced')
...
class AttendanceFormView(BaseAPIView):
	...
	def get(self, request, pk=None):
		try:
			...
			if pk is not None:
				proctorship_attendance = AttendanceForm.objects.filter(proctorship__id=pk)
			...
			return self.send_response(code='422', status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, description="No Attendance Form  matches the given query.")
		except AttendanceForm.DoesNotExist:
			return self.send_response(code='', status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, description="Attendance Form doesn't exists")
		except FieldError:
			return self.send_response(code=f'500', description="Cannot resolve keyword given in 'order_by' into field")
		except Exception as e:
			return self.send_response( code=f'500',description=e)
	...
	def post(self, request):
		try:
			...
			return self.send_response(code=f'422',status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,description="Attendance Form doesn't exists")
		except FieldError:
			return self.send_response( code=f'500',description="Cannot resolve keyword given in 'order_by' into field")
		except Exception as e:
			return self.send_response(code=f'500',description=e)
	...
	def put(self, request, pk:None):
		try:
			...
			serializer = AttendanceFormUpdateSerailizers(instance=proctorship_attendance, data = data)
			if serializer.is_valid():
				serializer.save()
				return self.send_response(success=True, code=f'201', status_code=status.HTTP_201_CREATED, description='Attendance Form is replaced')
			return self.send_response(code=f'422',status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, description=serializer.errors)
		except AttendanceForm.DoesNotExist:
			return self.send_response(code=f'422',status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,description="Attendance Form doesn't exists")
		except FieldError:
