...
from api.models import AreaOfExperties, EventType, Hcp_role, Hospital, Speciality
...
class SpeakerSerializer(serializers.ModelSerializer):
    ...
    focus_not_listed = serializers.CharField(
        required=True, allow_blank=True, allow_null=True
    )
    depart_employee = serializers.CharField(
        required=True, allow_blank=True, allow_null=True
    )
    name_employee = serializers.CharField(
        required=True, allow_blank=True, allow_null=True
    )
        ...
        try:
            with transaction.atomic():
                ...
                if "speech_focus_id" in validated_data.keys():
                    ...
                    if speech_focus_id:
                        validated_data["speech_focus"] = AreaOfExperties.objects.get(
                            id=speech_focus_id
                        )
...
class SpeakingEventSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        with transaction.atomic():
            ...
            status_data = {
                "status": StatusConstantData.objects.get(code="alternative-proposal"),
    ...
    def create(self, validated_data):
        try:
            with transaction.atomic():
                ...
                for each in speaker:
                    each["event"] = event
                    each["speaker_role"] = Hcp_role.objects.get(
                        code=each.pop("speaker_role")
                    )
                    each["specialty"] = Speciality.objects.get(
                        code=each.pop("specialty")
                    )
...
class StatusSerializer(serializers.ModelSerializer):
    ...
    def get_speaking_event(self, obj):
        try:
            return SpeakingEventViewSerializer(
                SpeakingEvent.objects.get(event_status__id=obj.id)
            ).data
        except:
            return None
    ...
    def get_status(self, obj):
        try:
            return StatusSerializer(
                EventStatus.objects.filter(event__id=obj.id), many=True
            ).data
        except:
            return None
    ...
    def create(self, validated_data):
        try:
            with transaction.atomic():
                ...
                EventStatus.objects.filter(event__id=validated_data["event_id"]).update(
                    is_active=False
                )
...
class SignedLetterSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        try:
            instance.signed_letter = validated_data.get(
                "signed_letter", instance.signed_letter
            )
            instance.save()
            return instance
        except Exception as e:
        ...
        try:
            if "speech_focus_id" in validated_data.keys():
                ...
                if speech_focus_id:
                    ...
                    instance.speech_focus = validated_data.get(
                        "speech_focus", instance.speech_focus
                    )
            if "proctor_id" in validated_data.keys():
                proctor_id = validated_data.pop("proctor_id")
                if proctor_id:
            ...
            if "speaker_role" in validated_data.keys():
                speaker_role = validated_data.pop("speaker_role")
                if speaker_role:
                    validated_data["speaker_role"] = Hcp_role.objects.get(
                        code=speaker_role
                    )
                    instance.speaker_role = validated_data.get(
                        "speaker_role", instance.speaker_role
                    )

            instance.duration = validated_data.get("duration", instance.duration)
            instance.title = validated_data.get("title", instance.title)
            instance.notes = validated_data.get("notes", instance.notes)
            instance.focus_not_listed = validated_data.get(
                "focus_not_listed", instance.focus_not_listed
            )
            instance.other_proctor = validated_data.get(
                "other_proctor", instance.other_proctor
            )
            instance.depart_employee = validated_data.get(
                "depart_employee", instance.depart_employee
            )
            instance.name_employee = validated_data.get(
                "name_employee", instance.name_employee
            )
    ...
    def create(self, validate_data):
        try:
            with transaction.atomic():
                ...
                speaking_evnt_feedback = SpeakingEventFeedBack.objects.create(
                    **validate_data
                )
                for each in rate_topic:
                    TopicRating.objects.create(
                        rate_topic=each["rate_topic"],
...
class SoeakingEventAttendanceSerailizers(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        try:
            with transaction.atomic():
                attendace_form = AttendanceFormSpeakingEvent.objects.create(
                    **validated_data
                )
                attendace_form.save()
...
class EventViewListingSerializer(serializers.ModelSerializer):
    ...
    def get_event_datails(self, obj):
        try:
            return SpeakingEventListingViewSerializer(
                SpeakingEvent.objects.filter(event_status__event__id=obj.id).latest(
                    "created_on"
                )
            ).data
        except:
    ...
    def get_speakers(self, obj):
        try:
            return SpeakerSerializer(
                Speaker.objects.filter(event__id=obj.id), many=True
            ).data
        except:
            return None
...
class CognosIdViewSerailizers(serializers.ModelSerializer):
    ...
    def get_cognos_id(self, obj):
        try:
            res = []
            for each in Hospital.objects.filter(
                cognos_id_speaker__event__id=obj.event.id
            ):
                dic = {}
                dic["id"] = each.id
...
class MeetingDocsSerailizers(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        try:
            instance.meeting_docs = validated_data.get(
                "meeting_docs", instance.meeting_docs
            )
            instance.save()
            return instance
        except Exception as e:
