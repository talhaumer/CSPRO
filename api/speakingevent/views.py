...
from api.speakingevent.models import SpeakingEvent, Speaker, Event, EventStatus, SpeakingEventFeedBack, \
    AttendanceFormSpeakingEvent, CognosId
from api.speakingevent.serializers import SpeakingEventSerializer, SpeakerSerializer, EventSerializer, \
    EventViewSerializer, StatusAddSerializer, SignedLetterSerializer, AgendaSerailizers, SpeakerUpdateSerializer, \
    SpeakingEventFeedBackSerializers, SoeakingEventAttendanceSerailizers, AttendanceSpeakingEventSerailizers, \
    AttendanceFormSpeakingEventUpdateSerailizers, EventViewListingSerializer, CognosIdSerailizers, \
    CognosIdUpdateSerailizers, CognosIdViewSerailizers, MeetingDocsSerailizers, AttendanceEventSerailizers, \
...
class SpeakerView(BaseAPIView):
    ...
    def get(self, request, pk=None):
        try:
            ...
            serializer = SpeakerSerializer(event, many=True)
            return self.send_response(success=True, status_code=status.HTTP_200_OK, payload=serializer.data, code='200',
                                      description='Details of serializer', log_description='', count=event.count())
        except ObjectDoesNotExist:
            return self.send_response(code='422', status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
    ...
    def post(self, request):
        try:
            ...
            if serializer.is_valid():
                ...
                data = serializer.instance.id
                return self.send_response(success=True, code=f'201', payload={"id": data},
                                          status_code=status.HTTP_201_CREATED, description='Speaker is created')
            return self.send_response(code=f'422', status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                                      description=serializer.errors)
        except Speaker.DoesNotExist:
            return self.send_response(code=f'422', status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
...
class SpeakingEventUpdateView(BaseAPIView):
    ...
    #       event = Event.objects.get(pk=pk)
    #       serializer = EventViewSerializer(event)
    #       return self.send_response(success=True,status_code=status.HTTP_200_OK, payload=serializer.data,code= '200',description='Details of serializer',log_description='')
    #     event = Event.objects.all()
    #     serializer = EventViewSerializer(event, many=True)
    #     return self.send_response(success=True,status_code=status.HTTP_200_OK, payload=serializer.data,code= '200',description='Details of serializer',log_description='', count = proctorship.count())
    #   except ObjectDoesNotExist:
    #     return self.send_response(code='422', status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, description="No Proctorship matches the given query.")
    #   except Event.DoesNotExist:
    #     return self.send_response(code='422', status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, description="proctorship doesn't exists")
    #   except FieldError:
    #     return self.send_response(code=f'500', description="Cannot resolve keyword given in 'order_by' into field")
    #   except Exception as e:
    #     return self.send_response( code=f'500',description=e)
...
class StatusSpeakingView(BaseAPIView):
    ...
    def get(self, request, pk=None):
        try:
            ...
            if activity_id:
                activity_status = EventStatus.objects.filter(event__id=activity_id).order_by("timestamp")
    ...
    def post(self, request):
        try:
            data = request.data
            data['user_id'] = request.user.id
            start_date = str(SpeakingEvent.objects.filter(event_status__event__id = data["event_id"]).latest("created_on").start_date)
            end_date = str(SpeakingEvent.objects.filter(event_status__event__id = data["event_id"]).latest("created_on").end_date)
            available_proctor = []
            not_available_proctor = []
            if data["status"] == "confirmed":
                proctors = Speaker.objects.filter(event__id=data["event_id"], status=True, revoke=False)
                if proctors:
                    for each in proctors:
                        if each.proctor:
                            if not check(each.proctor.id, start_date, end_date):
                                if not check_perceptorship(each.proctor.id, start_date, end_date):
                                    if not check_masterproctorship(each.proctor.id, start_date, end_date):
                                        if not check_event(each.proctor.id, start_date, end_date):
                    ...
                    if  available_proctor and not not_available_proctor:
                        Speaker.objects.filter(event__id=data["event_id"], proctor__id__in=available_proctor,
                                               revoke=False)
                        serializer = StatusAddSerializer(data=data)
                        if serializer.is_valid():
                            saved_status = serializer.save()
                        ...
                        return self.send_response(code=f'422', status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, description=des)
                else:
                    return self.send_response(code=f'422', status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
...
class SpeakingEventListing(BaseAPIView, CustomPagination):
    ...
    def get(self, request, pk=None):
        try:
            ...
            event = Event.objects.all()
            if search_value:
                event = event.filter(Q(id__icontains=search_value) |
                                     Q(activity_id__icontains=search_value) |
                                     Q(event_country__name__icontains=search_value) |
                                     Q(event_status_event__event_status__event_name__icontains=search_value) |
                                     ...
                                     Q(event_status_event__event_status__event_name__icontains=search_value) |
                                     Q(event_status_event__event_status__event_type__name__icontains=search_value) |
                                     Q(event_status_event__event_status__event_location__icontains=search_value) |
                                     Q(event_status_event__event_status__solution__solution__icontains=search_value) |
                                     Q(event_status_event__event_status__participants__icontains=search_value) |
                                     Q(event_status_event__event_status__audience_type__audience__icontains=search_value) |
                                     Q(event_status_event__event_status__audience_region__region__icontains=search_value) |
                                     Q(event_status_event__event_status__start_date__icontains=search_value)).distinct()
            ...
            event = event.filter(query_object).order_by(self.sorting_data(column_name,order)).distinct()
            # event = self.sorting_data(event, column_name)
            ...
            serializer = EventViewListingSerializer(event[offset: offset + limit], many=True)
            return self.send_response(success=True, status_code=status.HTTP_200_OK, payload=serializer.data, code='200',
                                      description='Speaking Event Details', log_description='', count=event.count())
        except Event.DoesNotExist:
            return self.send_response(code=f'422', status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                                      description="Speaking Event doesn't exists")
        except FieldError as e:
            return self.send_response(code=f'500', description=f'{e} Cannot resolve keyword given in order_by into field')
        except Exception as e:
            return self.send_response(code=f'500', description=e)
    ...
    def get(self, request, pk=None):
        try:
            if pk is not None:
                ...
                speaker.status = True
                speaker.save()
                return self.send_response(description="speaker is activate successfuly", success=True,
    ...
    def get(self, request, pk=None):
        try:
            if pk is not None:
                ...
                trainee.revoke = True
                trainee.save()
                return self.send_response(description="speaker is is revoked successfuly", success=True,
                                          status_code=status.HTTP_200_OK, code='200')
            return self.send_response(code=f'422', status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
...
class FeedBackSpeakingEvent(BaseAPIView):
    ...
    def get(self, request, pk=None):
        try:
            ...
            if event_id:
                event_feedback = SpeakingEventFeedBack.objects.filter(event__id=event_id)
                if search_value:
                    event_feedback = event_feedback.filter(Q(event__event_country__name__icontains=search_value) |
                                                           Q(speaker__proctor__user__name__icontains=search_value) |
                                                           Q(specialty__name__icontains=search_value) |
                                                           Q(country__name__icontains=search_value)).distinct()

                event_feedback = event_feedback.order_by(self.sorting_feedback(column_name, order))
                serializer = SpeakingEventFeedBackSerializers(event_feedback[offset: offset + limit], many=True)
                return self.send_response(success=True, status_code=status.HTTP_200_OK, payload=serializer.data,
                                          code='', description='Details of serializer', log_description='')
            if pk:
            ...
            return self.send_response(success=False, status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, payload=[],
                                      code='422', description='Enter Valid Speaking Id')
        except IntegrityError as i:
            return self.send_response(code=f'422', status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, description=str(i))
...
class AttendanceFormSpeakingEventView(BaseAPIView):
    ...
    def get(self, request, pk=None):
        try:
            if pk is not None:
                proctorship_attendance = AttendanceFormSpeakingEvent.objects.filter(event__id=pk)
                serializer = AttendanceSpeakingEventSerailizers(proctorship_attendance, many = True)
                return self.send_response(success=True, status_code=status.HTTP_200_OK, payload=serializer.data,
                                          code='', description='Details of serializer', log_description='')
    ...
    def post(self, request):
        try:
            ...
            data._mutable = True
            data["user_id"] = request.user.id
            type = SpeakingEvent.objects.filter(event_status__event__id=int(data["event_id"])).latest("created_on")
            if type.event_type.code == "speaking-at-corcym-webinar-hcp-peer-to-peer-event":
    ...
    def put(self, request, pk: None):
        try:
            data = request.data
            proctorship_attendance = AttendanceFormSpeakingEvent.objects.get(event__id=pk)
            serializer = AttendanceFormSpeakingEventUpdateSerailizers(instance=proctorship_attendance, data=data)
            if serializer.is_valid():
                serializer.save()
...
class CongosIdViewForm(BaseAPIView):
    ...
    def put(self, request, pk: None):
        try:
            ...
            serializer = CognosIdUpdateSerailizers(instance=proctorship_attendance, data=data)
            if serializer.is_valid():
                serializer.save()
                return self.send_response(success=True, code=f'201', status_code=status.HTTP_201_CREATED,
