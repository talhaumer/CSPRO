...
def send_email_sendgrid_template(
    from_email="", to_email="", subject="", data="", template=""
):
    try:
...
def check_perceptorship(proctor_id, start_date, end_date):
    try:
        ...
        precep_query_object &= Q(
            preceptorshipStatus_status__status__code="confirmed"
        ) | Q(preceptorshipStatus_status__status__code="waiting-for-docs")
        ...
        for x in percep_activities_id:
            latest_percep_purposal_id.append(
                PreceptorshipProposal.objects.filter(
                    status__preceptorship_activity__id=x
                )
                .latest("created_on")
                .id
            )
        ...
        for a in latest_percep_purposal_id:
            ...
            if (str(precp_proposal.start_date) < start_date) and (
                str(precp_proposal.end_date) > start_date
            ):
                precp_filtered.append(precp_proposal.id)
            elif (str(precp_proposal.start_date) < end_date) and (
                str(precp_proposal.end_date) > end_date
            ):
                precp_filtered.append(precp_proposal.id)
            elif (
                (str(precp_proposal.start_date) == start_date)
                or (str(precp_proposal.end_date) == end_date)
                or (str(precp_proposal.start_date) == end_date)
                or (str(precp_proposal.end_date) == start_date)
            ):
                precp_filtered.append(precp_proposal.id)
...
def check_masterproctorship(proctor_id, start_date, end_date):
    try:
        ...
        master_query_object &= Q(
            master_proctorship_status__status__code="confirmed"
        ) | Q(master_proctorship_status__status__code="waiting-for-docs")
        ...
        for x in master_activities_id:
            latest_master_purposal_id.append(
                MasterProctorshipProposal.objects.filter(
                    status__master_proctorship_activity__id=x
                )
                .latest("created_on")
                .id
            )
        ...
        for a in latest_master_purposal_id:
            ...
            elif (str(master_proposal.start_date) < end_date) and (
                str(master_proposal.end_date) > end_date
            ):
                master_filtered.append(master_proposal.id)
            elif (
                (str(master_proposal.start_date) == start_date)
                or (str(master_proposal.end_date) == end_date)
                or (str(master_proposal.start_date) == end_date)
                or (str(master_proposal.end_date) == start_date)
            ):
                master_filtered.append(master_proposal.id)
    ...
    try:
        ...
        for x in proctorship_id:
            latest_purposal_id.append(
                Proposal.objects.filter(status__proctorship_activity__id=x)
                .latest("created_on")
                .id
            )
        ...
        for a in latest_purposal_id:
            ...
            if (str(proposal.start_date) < start_date) and (
                str(proposal.end_date) > start_date
            ):
                filtered.append(proposal.id)
            elif (str(proposal.start_date) < end_date) and (
                str(proposal.end_date) > end_date
            ):
                filtered.append(proposal.id)
            elif (
                (str(proposal.start_date) == start_date)
                or (str(proposal.end_date) == end_date)
                or (str(proposal.start_date) == end_date)
                or (str(proposal.end_date) == start_date)
            ):
                filtered.append(proposal.id)

        proctorship_proctors = ProctorshipProctors.objects.filter(
            porposal__id__in=filtered, status=True
        )
...
def check_event(proctor_id, start_date, end_date):
    try:
        ...
        for each in event_id:
            speaking_event = SpeakingEvent.objects.filter(
                event_status__event__id=each
            ).latest("created_on")
            if (str(speaking_event.start_date) < start_date) and (
                str(speaking_event.end_date) > start_date
            ):
                filtered_event.append(each)
            elif (str(speaking_event.start_date) < end_date) and (
                str(speaking_event.end_date) > end_date
            ):
                filtered_event.append(each)
            elif (
                (str(speaking_event.start_date) == start_date)
                or (str(speaking_event.end_date) == end_date)
                or (str(speaking_event.start_date) == end_date)
                or (str(speaking_event.end_date) == start_date)
            ):
                filtered_event.append(each)

        speakers = Speaker.objects.filter(
            event__id__in=filtered_event,
...
def check_mics_perceptorship(proctor_id, start_date, end_date):
    try:
        ...
        master_query_object &= Q(
            mics_preceptorshipStatus_status__status__code="confirmed"
        ) | Q(mics_preceptorshipStatus_status__status__code="waiting-for-docs")
        ...
        for x in master_activities_id:
            latest_master_purposal_id.append(
                MicsPreceptorshipProposal.objects.filter(
                    status__mics_preceptorship_activity__id=x
                )
                .latest("created_on")
                .id
            )
...
def check_mics_perceptorship(proctor_id, start_date, end_date):
    try:
        ...
        for x in master_activities_id:
            latest_master_purposal_id.append(
                MicsProctorshipProposal.objects.filter(
                    status__proctorship_activity__id=x
                )
                .latest("created_on")
                .id
            )
...
def available_proctors(proctors, start_date, end_date):
    try:
        if proctors:
            ...
            proctorship_proctors_id = [
                each.proctors.id for each in proctorship_proctors
            ]
            proctorship_proctors_id = list(set(proctorship_proctors_id))
...
def solo_memo(activity_id):
    if MemoProctorFeedBack.objects.filter(
        proctorship__id=activity_id
    ) and SoloSmartProctorFeedBack.objects.filter(proctorship__id=activity_id):
        return True
    else:
        return False
...
def memo_perc(activity_id):
    if MemoProctorFeedBack.objects.filter(
        proctorship__id=activity_id
    ) and ProctorshipProctorFeedback.objects.filter(proctorship__id=activity_id):
        return True
    else:
        return False
...
def perc_solo(activity_id):
    if ProctorshipProctorFeedback.objects.filter(
        proctorship__id=activity_id
    ) and SoloSmartProctorFeedBack.objects.filter(proctorship__id=activity_id):
        return True
    else:
...
def check_product_feedback(activity_id, prod=None):
    ...
    if Proctorship.objects.get(id=activity_id).secondary_product:
        product.append(
            Proctorship.objects.get(id=activity_id).secondary_product.product_name
        )
...
def perceptership(obj):
    if (
        AttendanceFormPerceptorship.objects.filter(preceptorship__id=obj.id)
        and InvoicePerceptorship.objects.filter(preceptorship__id=obj.id)
        and PreceptorshipTraineeUploads.objects.filter(
            preceptorship_trainee__preceptorship__id=obj.id,
            preceptorship_trainee__revoke=False,
        ).count()
        == TraineePreceptorshipProfile.objects.filter(preceptorship__id=obj.id).count()
        and PreceptorshipStatus.objects.filter(preceptorship_activity__id=obj.id)
        .order_by("-id")[0]
        .status.code
        != "closed"
    ):
        PreceptorshipStatus.objects.filter(preceptorship_activity__id=obj.id).update(
            is_active=False
        )
        status_obj = {}
...
def proctorship(obj):
    if (
        check_product_feedback(obj.id, prod="Solo Smart")
        and TraineeProfile.objects.filter(proctorship__id=obj.id, revoke=False).count()
        == TraineeFeedback.objects.filter(
            trainee__proctorship__id=obj.id, trainee__revoke=False
        ).count()
        and AttendanceForm.objects.filter(proctorship__id=obj.id)
        and Invoice.objects.filter(proctorship__id=obj.id)
        and Status.objects.filter(proctorship_activity__id=obj.id)
        .order_by("-id")[0]
        .status.code
        != "closed"
    ):
        ...
        status_data = {
            "status": StatusConstantData.objects.get(code="closed"),
            "proctorship_activity": Proctorship.objects.get(id=obj.id),
        }
...
def master_proctorship(obj):
    if (
        MasterProctorshipTraineeProfile.objects.filter(
            master_proctorship__id=obj.id, revoke=False
        ).count()
        == MasterProctorshipProctorReport.objects.filter(
            master_proctorship_trainee__master_proctorship__id=obj.id,
            master_proctorship_trainee__revoke=False,
        ).count()
        and MasterProctorshipFeedback.objects.filter(
            master_proctorship_activity__id=obj.id
        )
        and InvoiceMasterProctorShip.objects.filter(master_proctorship__id=obj.id)
        and AttendanceFormMasterProctorShip.objects.filter(
            master_proctorship__id=obj.id
        )
        and MasterProctorshipStatus.objects.filter(
            master_proctorship_activity__id=obj.id
        )
        .order_by("-id")[0]
        .status.code
